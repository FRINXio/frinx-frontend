...
class Workflow2Graph {
    ...
    _convert(wfe = {}, meta = {}) {
        ...
        for(let v in this.vertices) {
            ...
            if (status !== '') {
                ...
                let tooltip = '<p><strong>Input</strong></p>' + JSON.stringify(et.input, null, 2) + '<p/><p><strong>Output</strong></p>' + JSON.stringify(et.output, null, 2);
    ...
    getNodes(vertices, nodes, t1, t2, forks, subworkflows, isExecutingCase){
        ...
        if(t1.type === 'final' && t1.type === t2.type){
            vertices[t1.taskReferenceName] = {name: t1.name, ref: t1.taskReferenceName, type: 'simple', style: 'fill:#ffffff', shape: 'circle', system: true};
            return nodes;
        }
