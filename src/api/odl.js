...
router.put('/mount/:topology/:node', async (req, res, next) => {
    try {
        await http.put(odlConfigURL + "/" + req.params.topology + "/node/" + req.params.node, req.body, authToken);
...
router.delete('/unmount/:topology/:node', async (req, res, next) => {
    try {
        const result = await http.delete(odlConfigURL + "/" + req.params.topology + "/node/" + req.params.node, req.body, authToken);
...
router.post('/operations/replace-config-with-operational', async (req, res, next) => {
    try {
        const result = await http.post(odlOperationsURL + "/uniconfig-manager:replace-config-with-operational", req.body, authToken);
        res.status(200).send(result);
    } catch (e) {
        next(e);
    }
...
router.post('/operations/replace-config-with-snapshot', async (req, res, next) => {
    try {
