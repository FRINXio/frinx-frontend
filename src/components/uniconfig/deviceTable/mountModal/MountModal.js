...
class MountModal extends Component {
  ...
  componentWillMount() {
    ...
    this.setState({
      ...
      mountCliFormAdv: {
        ...JSON.parse("[" + mountCliTemplateAdv + "]")[0],
        ...JSON.parse("[" + mountCliTemplateLazyOFF + "]")[0]
      },
      mountNetconfForm: JSON.parse("[" + mountNetconfTemplate + "]")[0],
      mountNetconfFormAdv: JSON.parse("[" + mountNetconfTemplateAdv + "]")[0],
      mountNetconfFormCaps: JSON.parse(
        "[" + mountNetconfTemplateCapabilities + "]"
      )[0],
      deviceType: JSON.parse("[" + mountCliTemplate + "]")[0][
  ...
  mountDevice(node, payload, topology) {
    ...
    http
      .put("/api/odl/mount/" + topology + "/" + node, payload)
      .then(res => {
        console.log(res);
      })
      .then(() => {
  ...
  handleBlacklist() {
    if (this.state.enableBlacklist) {
      ...
      this.setState({
        mountNetconfFormAdv: {
          ...this.state.mountNetconfFormAdv,
          ...
          <Col>
            ...
            <InputGroup>
              <InputGroup.Append style={{ width: "40px" }}>
                <InputGroup.Checkbox
                  checked={this.state.enableBlacklist}
                  onChange={this.handleBlacklist.bind(this)}
                />
              </InputGroup.Append>
              <Form.Control
                disabled={!this.state.enableBlacklist}
                as="textarea"
                rows={2}
            ...
            <Form.Text className="text-muted">
              List of blacklisted root paths that should not be read from the
              device
            </Form.Text>
          </Col>
        </Row>
  ...
  handleToggle(value) {
    ...
    if (this.state.activeToggles.includes(value)) {
      ...
      Object.keys(JSON.parse("[" + valueToFormArr[value][1] + "]")[0]).filter(
        key => {
          if (key in topologyState) {
            return delete topologyState[key];
          }
      ...
      this.setState({
        ...
        [topologyForm]: {
          ...topologyState,
          ...JSON.parse("[" + valueToFormArr[value][0] + "]")[0]
        }
      });
    } else {
      Object.keys(JSON.parse("[" + valueToFormArr[value][0] + "]")[0]).filter(
        key => {
          if (key in topologyState) {
            return delete topologyState[key];
          }
      ...
      this.setState({
        ...
        [topologyForm]: {
          ...topologyState,
          ...JSON.parse("[" + valueToFormArr[value][1] + "]")[0]
        }
      });
    }
  }
  ...
  render() {
    ...
    let capModal = this.state.showCapModal ? (
      <CapModal
        getUpdatedCaps={this.getUpdatedCaps.bind(this)}
        caps={this.state.mountNetconfFormCaps}
        modalHandler={this.showCapModal.bind(this)}
        show={this.state.showCapModal}
      />
    ) : null;
    ...
    const settingsGroup = () => {
      return (
        ...
        >
          <Button
            onClick={() => this.setState({ isAdv: false })}
            style={{ width: "50%" }}
            active={this.state.isAdv ? null : "active"}
            className="noshadow"
          ...
          <Button
            onClick={() => this.setState({ isAdv: true })}
            style={{ width: "50%" }}
            active={this.state.isAdv ? "active" : null}
            className="noshadow"
            variant="outline-primary"
          >
      ...
      return this.state.isAdv ? (
        ...
        >
          <Button
            ...
            style={{ width: "50%" }}
            active={this.state.activeToggles.includes(0) ? "active" : null}
            className="noshadow"
          ...
          <Button
            ...
            style={{ width: "50%" }}
            active={this.state.activeToggles.includes(1) ? "active" : null}
            className="noshadow"
            variant="outline-info-toggle"
          >
            Dry-run
      ...
      return this.state.isAdv ? (
        ...
        >
          ...
          <Button
            ...
            style={{ width: "50%" }}
            active={this.state.activeToggles.includes(2) ? "active" : null}
            className="noshadow"
            variant="outline-info-toggle"
          >
            UniConfig Native
    ...
    const transportField = (item, i, type) => {
      ...
      return (
        <Form.Group controlId={`mount${type}Input-${item[0].split(":").pop()}`}>
    ...
    const deviceTypeVersionField = (item, i, type, which) => {
      ...
      if (which === "type") {
        Object.values(this.state.deviceTypeVersion).map(obj => {
          return options.push({
            value: obj["device-type"],
            label: obj["device-type"]
          });
        });
      } else {
        Object.values(this.state.deviceTypeVersion).map(obj => {
          if (obj["device-type"] === this.state.deviceType) {
            return options.push({
              value: obj["device-version"],
              label: obj["device-version"]
            });
          }
          return null;
        });
      ...
      return (
        <Form.Group controlId={`mount${type}Input-${item[0].split(":").pop()}`}>
          ...
          <Dropdown
            options={options}
            onChange={e => this.handleInput(e, i, formToDisplay, which)}
            value={item[1][0]}
          />
          <Form.Text className="text-muted">{item[1][1]}</Form.Text>
        </Form.Group>
          ...
          <InputGroup>
            ...
            >
              <InputGroup.Text>
                <i
                  className={
                    this.state.showPass ? "fas fa-eye-slash" : "fas fa-eye"
                  }
                />
              </InputGroup.Text>
            </InputGroup.Append>
        ...
        <Form.Group controlId={`mount${type}Input-${item[0].split(":").pop()}`}>
          ...
          <InputGroup>
            <InputGroup.Append
              style={{ width: "40px" }}
              className="clickable"
              onClick={this.showCapModal.bind(this)}
            >
              <InputGroup.Text>
                <i className="fas fa-plus-circle" />
              </InputGroup.Text>
            </InputGroup.Append>
            <Form.Control type="input" disabled value={item[1][0]} />
          </InputGroup>
          <Form.Text className="text-muted">
            &nbsp;&nbsp;
      ...
      return (
        <Form.Group controlId={`mount${type}Input-${item[0].split(":").pop()}`}>
          ...
          <ButtonGroup style={{ marginBottom: "20px" }} className="d-flex">
            <Button
              onClick={() => this.handleInput(false, i, formToDisplay)}
              style={{ width: "50%" }}
              active={item[1][0] ? null : "active"}
              className="noshadow"
              variant="outline-primary"
            >
              false
            ...
            <Button
              onClick={() => this.handleInput(true, i, formToDisplay)}
              style={{ width: "50%" }}
              active={item[1][0] ? "active" : null}
              className="noshadow"
              variant="outline-primary"
            >
              true
    ...
    const whichField = (item, i, type) => {
      ...
      switch (id) {
        case "password":
          return passwordField(item, i, type);
        case "transport-type":
          return transportField(item, i, type);
        case "override":
          return capabilitiesField(item, i, type);
        case "device-type":
          return deviceTypeVersionField(item, i, type, "type");
        case "device-version":
          return deviceTypeVersionField(item, i, type, "version");
        case "reconcile":
          return booleanField(item, i, type);
        case "tcp-only":
          return booleanField(item, i, type);
        default:
          return inputField(item, i, type);
      }
    };
    ...
    const connectionBtn = () => {
      return (
          ...
          variant={
            this.state.connectionStatus === null
              ...
              : this.state.connectionStatus === "connecting" ||
                this.state.connectionStatus.startsWith("Sending")
              ? "info"
        ...
        >
          {this.state.connectionStatus === null ? null : this.state
              .connectionStatus === "connecting" ||
            this.state.connectionStatus.startsWith("Sending") ? (
    ...
    return (
      <Modal size="lg" show={this.state.show} onHide={this.handleClose}>
        ...
        <Modal.Body style={{ padding: "30px" }}>
          <Tabs
            onSelect={this.changeMountType.bind(this)}
            style={{ marginBottom: "20px" }}
            defaultActiveKey={this.state.mountType}
            id="mountTabs"
          >
            ...
            <Tab eventKey="Netconf" title="Netconf">
              ...
              {this.state.activeToggles.includes(2) && this.state.isAdv
                ? this.showNativeSettings()
                : null}
            </Tab>
          </Tabs>
        </Modal.Body>
