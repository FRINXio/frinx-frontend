export function parseResponse(type, body) {
  ...
  switch (type) {
    case "dryrun":
      return parseDryRun(type, bodyJSON);
    case "sync":
      return parseSync(type, bodyJSON);
    case "commit":
      return parseCommit(type, bodyJSON);
    case "replacesnap":
      return parseReplaceSnap(type, bodyJSON);
    case "replaceconf":
      return parseReplaceConf(type, bodyJSON);
    default:
      break;
  }
}
...
function parseDryRun(type, bodyJSON) {
  ...
  if (bodyJSON["output"]["node-config-results"]) {
    nodeStatus =
      bodyJSON["output"]["node-config-results"]["node-config-result"]["0"][
    ...
    configuration = bodyJSON["output"]["node-config-results"][
      "node-config-result"
    ]["0"]["cli-configuration"]
      ? bodyJSON["output"]["node-config-results"]["node-config-result"]["0"][
          "cli-configuration"
...
function parseSync(type, bodyJSON) {
  ...
  try {
    if (bodyJSON["output"]["node-sync-results"]["node-sync-result"]) {
      nodeId =
        bodyJSON["output"]["node-sync-results"]["node-sync-result"][0][
          "node-id"
        ];
      status = "completed";
      errorMessage =
