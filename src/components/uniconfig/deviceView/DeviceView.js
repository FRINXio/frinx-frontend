...
class DeviceView extends Component {
  ...
  syncFromNetwork() {
    ...
    http
      .post("/api/odl/operations/sync-from-network", target)
      .then(res_first => {
        http.get("/api/odl/oper/uniconfig/" + this.state.device).then(res => {
          this.setState({
  ...
  replaceConfig() {
    ...
    http
      .post("/api/odl/operations/replace-config-with-operational", target)
      .then(res => {
        console.log(res);
        this.refreshConfig();
        this.setState({
  ...
  loadSnapshot(snapshotId) {
    ...
    if (this.state.deletingSnaps) {
      ...
      http
        .post("/api/odl/operations/replace-config-with-snapshot", target)
        .then(res_first => {
          http
            .get(
              "/api/odl/conf/snapshots/" +
                snapshotName +
                "/" +
                this.state.device
            )
