...
export class WorkflowDiagram {
  ...
  getMatchingNode(taskName) {
    return _.toArray(this.nodes).find(
  ...
  linkDecisionNodes() {
    this.nodes.forEach((node, pos) => {
      if (node.type === "decision") {
        ...
        let neutralNode = this.nodes[pos + decisionCaseTasksArray.length + 1];

        // connect decision -> first nodes
        firstInBranch.forEach((firstNode, k) => {
          let whichPort = k === 0 ? "failPort" : "completePort";
          this.links.push(this.linkNodes(node, firstNode, whichPort));
        });
        ...
        if (neutralNode) {
          ...
          this.links.push(this.linkNodes(node, neutralNode, "neutralPort"));
        }
      }
    });
  }
  ...
  calculatePosition(branchX, branchY) {
    ...
    if (_.isEmpty(nodes)) {
      ...
      x =
        this.mostRightNodeX() + this.getNodeWidth(nodes[nodes.length - 1]) + 50;
      y = startPos.y;
    }
