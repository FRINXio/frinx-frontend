  ...
  switch (task.type) {
    case "SUB_WORKFLOW": {
      let posX =
        nodes.length === 0
          ? startPosition.x
          : find_mostRightNode(nodes) +
            getNodeWidth(nodes[nodes.length - 1]) +
            50;
    ...
    case "FORK_JOIN": {
      ...
      let branchSpread =
        (branchCount * 47 + (branchCount - 1) * branchMargin) / forkDepth; //branches size in parallel
        ...
        branch.forEach((branchTask, k) => {
          let branchPosX =
            branchTask.type === "JOIN"
              ? find_mostRightNode(nodes) + 220
              : posX + 220 + k * (getNodeWidth(nodes[nodes.length - 1]) + 50);
    ...
    case "DECISION": {
      ...
      // branches size in parallel - the deeper the fork node, the smaller the spread and margin is
      let branchSpread =
        (caseCount * 47 + (caseCount - 1) * branchMargin) / forkDepth; //branches size in parallel
...
export const transform_workflow_to_diagram = (
  definition,
  startPosition,
  app
) => {
