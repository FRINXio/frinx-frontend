...
describe('Create workflow, test and delete it', () => {
  ...
  it('workflow builder', () => {
    ...
    cy.intercept('PUT', '/api/workflow/metadata/workflow', { fixture: 'workflow-builder/01put.json' }).as(
      'put_metadata',
    );
    // 'workflow builder': 8
  ...
  it('save changes check', () => {
    ...
    cy.get('input[placeholder="Search by keyword."]').type('test workflow');
    cy.get('a[href="/workflow-manager/builder/test workflow copy/1"]').click();
