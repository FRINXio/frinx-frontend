  ...
  beforeEach(() => {
    cy.intercept('POST', 'http://localhost:3000/api/resource', (req) => {
      if (req.body.hasOwnProperty('query') && hasOperationName(req, 'GetPools')) {
        req.reply({ fixture: 'resource-manager/pools/NITRA_KLOKOCINA/GetPools.json' });
      }
    }).as('getPools');
    cy.intercept('POST', 'http://localhost:3000/api/resource', (req) => {
      if (req.body.hasOwnProperty('query') && hasOperationName(req, 'GetResourceTypes')) {
        req.reply({ fixture: 'resource-manager/pools/NITRA_KLOKOCINA/GetResourceTypes.json' });
      }
    }).as('GetResourceTypes');
  ...
  it('Create pool, Claim and create nested pool', () => {
    ...
    cy.intercept('POST', 'http://localhost:3000/api/resource', (req) => {
      if (req.body.hasOwnProperty('query') && hasOperationName(req, 'SelectAllocationStrategies')) {
        req.reply({ fixture: 'resource-manager/pools/NITRA_KLOKOCINA/CreatePool/SelectAllocationStrategies.json' });
      }
    }).as('SelectAllocationStrategies');

    cy.intercept('POST', 'http://localhost:3000/api/resource', (req) => {
      if (req.body.hasOwnProperty('query') && hasOperationName(req, 'SelectResourceTypes')) {
        req.reply({ fixture: 'resource-manager/pools/NITRA_KLOKOCINA/CreatePool/SelectResourceTypes.json' });
      }
    }).as('SelectResourceTypes');

    cy.intercept('POST', 'http://localhost:3000/api/resource', (req) => {
      if (req.body.hasOwnProperty('query') && hasOperationName(req, 'RequiredPoolProperties')) {
        req.reply({ fixture: 'resource-manager/pools/NITRA_KLOKOCINA/CreatePool/RequiredPoolProperties.json' });
      }
    ...
    cy.intercept('POST', 'http://localhost:3000/api/resource', (req) => {
      if (req.body.hasOwnProperty('query') && hasOperationName(req, 'GetPools')) {
        req.reply({ fixture: 'resource-manager/pools/NITRA_KLOKOCINA/FormCreatePool/GetPools.json' });
      }
    }).as('GetPools');
    ...
    cy.intercept('POST', 'http://localhost:3000/api/resource', (req) => {
      if (req.body.hasOwnProperty('query') && hasOperationName(req, 'GetResourceTypeByName')) {
        req.reply({ fixture: 'resource-manager/pools/NITRA_KLOKOCINA/Config/GetResourceTypeByName.json' });
      }
    }).as('GetResourceTypeByName');

    cy.intercept('POST', 'http://localhost:3000/api/resource', (req) => {
      if (req.body.hasOwnProperty('query') && hasOperationName(req, 'AllocatedResources')) {
        req.reply({ fixture: 'resource-manager/pools/NITRA_KLOKOCINA/Config/AllocatedResources.json' });
      }
...
//     cy.get('[id="isNested-label"]').click();  // Nested
    ...
    cy.intercept('POST', 'http://localhost:3000/api/resource', (req) => {
      if (req.body.hasOwnProperty('query') && hasOperationName(req, 'ClaimResourceWithAltId')) {
        req.reply({ fixture: 'resource-manager/pools/NITRA_KLOKOCINA/Claim/ClaimResourceWithAltId.json' });
      }
    ...
    cy.intercept('POST', 'http://localhost:3000/api/resource', (req) => {
      if (req.body.hasOwnProperty('query') && hasOperationName(req, 'AllocatedResources')) {
        req.reply({ fixture: 'resource-manager/pools/NITRA_KLOKOCINA/Claim/AllocatedResources.json' });
      }
    }).as('AllocatedResources');
    ...
    cy.intercept('POST', 'http://localhost:3000/api/resource', (req) => {
      if (req.body.hasOwnProperty('query') && hasOperationName(req, 'CreateNestedAllocationPool')) {
        req.reply({ fixture: 'resource-manager/pools/NITRA_KLOKOCINA/CreateNested/CreateNestedAllocationPool.json' });
      }
    }).as('CreateNestedAllocationPool');
    cy.intercept('POST', 'http://localhost:3000/api/resource', (req) => {
      if (req.body.hasOwnProperty('query') && hasOperationName(req, 'GetPools')) {
        req.reply({ fixture: 'resource-manager/pools/NITRA_KLOKOCINA/CreateNested/GetPools.json' });
      }
    }).as('GetPools');
