...
async function getWorkflowExecOutput<T>(workflowId: string, abortController: AbortController): Promise<T | null> {
  const { getWorkflowInstanceDetail } = uniflowCallbackUtils.getCallbacks;
  try {
