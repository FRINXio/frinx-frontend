...
const UNMOUNT_NODE_URL = (topology, node_id) => "/rests/data/network-topology:network-topology/topology=" + topology + "/node=" + node_id;
...
const DeviceList = (props) => {
    ...
    const fetchData = async () => {
        ...
        let nodesCli = await Promise.all((topologyCli?.node || []).map(async node => {
            return await createNodeObject(topologyCli["topology-id"], node)
        }));

        let nodesNetconf = await Promise.all((topologyNetconf?.node || []).map(async node => {
    ...
    const updateNode = async (node) => {
        let result = await http.get(global.backendApiUrlPrefix + GET_NODE_URL(node.topologyId, node.nodeId), global.authToken);
    ...
    const unmountNodes = async () => {
        let unmounted = await Promise.all(checked.map(async node => {
    ...
    const unmountNode = async (node) => {
        return await http.delete(global.backendApiUrlPrefix + UNMOUNT_NODE_URL(node.topologyId, node.nodeId), global.authToken);
    };
    ...
    return (
        <Container>
            <DeviceListHeader title={"Devices"} checked={checked} fetchData={fetchData} mountNode={mountNode}
                              unmountNodes={unmountNodes}/>
            <DeviceFilters nodes={nodes} setQuery={setQuery} setOsVersion={setOsVersion}/>
            <DeviceTable nodes={filteredNodes} checked={checked} setChecked={setChecked} updateNode={updateNode}/>
        </Container>
    )
};
