...
const NetconfTab = ({templateNode}) => {
    ...
    const mountNetconfDevice = async () => {
        ...
        const dryRunOn = {
            "netconf-node-topology:dry-run-journal-size": parseInt(netconfMountAdvForm["netconf-node-topology:dry-run-journal-size"])
        };

        const overrideCapabilitiesOn = {
            "netconf-node-topology:yang-module-capabilities": netconfMountAdvForm["netconf-node-topology:yang-module-capabilities"],
        };
        ...
        const result = await http.put(global.backendApiUrlPrefix + MOUNT_NETCONF_DEVICE_URL(nodeId), payload, global.authToken);
        const {statusCode, statusText} = result;
    ...
    const checkConnectionStatus = async (nodeId) => {
        const result = await http.get(global.backendApiUrlPrefix + GET_NETCONF_NODE_NONCONFIG_URL(nodeId), global.authToken);
                    ...
                    (netconfMountAdvForm[key] ? on : off)?.map(({displayValue, key}) =>
                        <Grid key={displayValue} item xs={size}>
                            <TextField
                                ...
                                helperText={description}
                                onChange={(e) => setNetconfMountAdvForm({
                                    ...netconfMountAdvForm,
                                    [key]: e.target.value
                                })}
                                variant="outlined"
